...
\begin{align*}
\emph{zero} &: \type{Contr}\\
\emph{transfer} &: \type{Party} \times \type{Party} \times \type{Currency}\to \type{Contr}\\
\emph{scale}&: \type{Expr}_\reals \times \type{Contr} \to \type{Contr}\\
\emph{translate}&: \nats \times \type{Contr} \to \type{Contr}\\ 
\emph{checkWithin}&:  \type{Expr}_\bools \times \nats \times \type{Contr} \times \type{Contr} %\\&
\to \type{Contr}\\
\emph{both}&: \type{Contr} \times \type{Contr} \to \type{Contr}
\end{align*}
\vspace*{-3ex}
}
...
{\small
\[
\emph{acc} : (\type{Expr} \to \type{Expr}) \to \nats \to \type{Expr} \to \type{Expr}
\]
}
\vspace*{-3ex}

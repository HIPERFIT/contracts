...
Expressions which involve observables can be translated (similar to \cd{transl} for contracts).
%
...
A expression is \emph{certain} if it does not depend on observables.
\medskip
...
$\ttt{flow}$ is a cash flow: \cd{(amount: real, c: currency, from,to: party)}

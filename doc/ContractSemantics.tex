...
\jbcomment{Need to explain $ exp/d$ notation at this point.}
...
\begin{frame}
    \frametitle{Contract Semantics -- Equivalences}
{\footnotesize
...
\begin{align*}
    ...
    \checkWithin(b,0,c_1,c_2)   &= \ifff(b,c_1,c_2) \ \ast\\
\end{align*}
\end{columns}
...
\transl(d,\checkWithin (b, e, c_1, c_2))  &=  \\
                             \checkWithin &(b/d, e, \transl(d,c1), \transl(d,c2))\\[1ex]
\mbox{with }\ast:\   \transl(d,\ifff(b,c_1,c_2))  &=  \ifff(b/d, transl(d,c_1), transl(d,c_2))\\
\end{align*}
...
An environment $\ttt{env}$ stores all fixings for observables and choices, and is used in the \ldots


\ldots semantics of an expression: $ \mathcal{E} : \ttt{exp0} \times \ttt{env} \rightarrow V$\\
{\footnotesize
(where $V$ is the set of all expression values, \cd{int}, \cd{real}, \cd{bool}).
}
